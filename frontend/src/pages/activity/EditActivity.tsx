    ...
    useEffect(() => {
        if (activity && categories && categories.length > 0 && !isLoadingCategories) {
            ...
            if (activity.imageUrl) {
                // If there's an existing image, it's the cover (conceptually)
                // We don't set imagePreview (which is for NEW file), but we render expected behavior in JSX
                // However, EditCampArea uses imagePreview slightly differently or relies on prop.
                // Let's stick to the pattern:
                // imagePreview is for LOCALLY selected file.
                // We will use a derived state or logic in render for "currentCover".
            }
    ...
    const onSubmit = async (data: ActivityFormValues) => {
        try {
            ...
            const payload = {
                ...
                imageUrl: coverUrl,
                keptImages: existingImages, // Existing URLs to keep
                additionalImages: newAdditionalUrls, // Newly uploaded URLs
                        ...
                        <CardContent className="p-6 md:p-8">
                            <Form {...form}>
                                <form
                                    ...
                                    className="space-y-6"
                                >
                                    {/* Image Upload Grid - Moved to Top */}
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            ...
                                            <div className="relative aspect-square rounded-2xl overflow-hidden group border-2 border-orange-500 bg-gray-100">
                                                {(imagePreview || activity?.imageUrl) ? (
                                                    <img
                                                        src={imagePreview || activity?.imageUrl}
                                                        alt="Cover"
                                        ...
                                        >
                                            {isSubmitting ? (
                                                <>
                                                    ...
                                                    {uploadStatus ? uploadStatus : "Simpan Perubahan"}
                                                </>
                                            ) : (
                                                "Simpan Perubahan"
                                            )}
